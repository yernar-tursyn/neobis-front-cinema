...
function showMovies(data) {
    ...
    const likedFilms = data.films.filter(movie => likedMovies.includes(movie.nameRu));
    console.log(data.films);
    if (likedFilms.length > 0) {
      likedFilms.forEach((movie) => {
      ...
      const noLikedMoviesEl = document.createElement("div");
      noLikedMoviesEl.classList.add("no-liked-movies");
      noLikedMoviesEl.textContent = "У вас нет понравившихся фильмов.";
      moviesEl.appendChild(noLikedMoviesEl);
    }
  }
